...
const CompletedTodos = ({ todos , deleteTodos }) => {


  const totalTodos = todos.filter((todo) => todo.completed === true).length

  return (
    <section className="flex h-[520px] w-full max-w-md flex-col overflow-hidden rounded-xl bg-[#e9edf5] p-5 font-sans text-[#293655] shadow-[10px_10px_20px_#c2cad9,-10px_-10px_20px_#ffffff]">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] font-extrabold tracking-[0.15em] text-[#d72e52]">
            OVERVIEW
          </p>
          <h2 className="mt-1 text-xl font-extrabold">Completed Tasks</h2>
        </div>
                ...
                <article className="rounded-2xl bg-[#e9edf5] p-4 shadow-[4px_4px_8px_#c2cad9,-4px_-4px_8px_#ffffff]">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    onClick={() => deleteTodos(todo._id)}
                      ...
                      className="grid h-11 w-11 place-items-center rounded-2xl text-[#ff3655] shadow-[4px_4px_8px_#c2cad9,-4px_-4px_8px_#ffffff]"
                    >
                      <svg
                        className="h-5 w-5"
                        viewBox="0 0 24 24"
